...
export class WeeklyReportsPageComponent {
  ...
  readonly classRows = computed<ClassReportRow[]>(() => {
    ...
    return this.activeClasses().map((schoolClass) => {
      ...
      const excusedStudents = this.classStudentsWithStatus(students, 'EXCUSED');
  ...
  readonly classReportSummary = computed<ClassReportSummary>(() => {
    if (this.selectedClassReportDay() === '-') {
      ...
      const presentStudents = Math.max(0, totalStudents - studentsWithAbsences);
    ...
    if (this.selectedView() === 'classes') {
      ...
      const classRows = [
        ...
        [
          ...
          `${summary.studentPercent}%`,
          summary.absentStudents,
          `${summary.absentPercent}%`,
          summary.excusedAbsences,
          `${summary.excusedPercent}%`,
          summary.sickAbsences,
          `${summary.sickPercent}%`,
          summary.noReasonAbsences,
          `${summary.noReasonPercent}%`,
          summary.totalMeals,
          `${summary.mealPercent}%`,
        ],
        [],
        ['Клас', 'Присутні', '%', 'Всього відсутніх', '%', 'п/п', '%', 'хв.', '%', 'б.п.', '%', 'Харчувались', '%'],
        ...classReportRows,
      ];
      ...
      classSheet['!autofilter'] = { ref: `A7:M${Math.max(7, classRows.length)}` };
